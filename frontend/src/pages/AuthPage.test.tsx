...
describe("AuthPage", () => {
	...
	it("logs in with email and password", async () => {
		vi.mocked(authApi.login).mockResolvedValue({ token: "fake-token",
			refreshToken: "fake-refresh-token", email: "user@example.com", language: "en", defaultCurrency: "EUR", interviewReminderMode: "ALWAYS", interviewReminderHours: 24, interviewReminderPrompted: false });
		renderAuthPage();
	...
	it("switches to the signup form and registers", async () => {
		vi.mocked(authApi.register).mockResolvedValue({ token: "fake-token",
			refreshToken: "fake-refresh-token", email: "new@example.com", language: "en", defaultCurrency: "EUR", interviewReminderMode: "ALWAYS", interviewReminderHours: 24, interviewReminderPrompted: false });
		renderAuthPage();
